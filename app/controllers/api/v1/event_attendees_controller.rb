class Api::V1::EventAttendeesController < ApplicationController
  ...
  def mark_attendance
    ...
    unless params[:otp].present? || params[:email].present? || params[:name].present? || params[:phone].present?
      return render json: { error: 'OTP, email, name, or phone is required' }, status: :unprocessable_entity
    end
    ...
    if success
      render json: {
        ...
        attendee: attendee.is_a?(EventAttendee) ? EventAttendeeSerializer.new(attendee) : EventQuickRegistrationSerializer.new(attendee)
      }, status: :ok
    else
      render json: { errors: attendee.errors.full_messages }, status: :unprocessable_entity
    end
